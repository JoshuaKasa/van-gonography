...
$$
\begin{align*}
&\text{We can also see this as a simple matrix operation:} \\
&\text{Let } I \text{ be an } m \times n \text{ matrix representing an image, where each element } p_{ij} \text{ corresponds to a pixel value represented as a hexadecimal number } pv. \text{ This hexadecimal number is decomposed into three channels } R \text{ (red), } G \text{ (green), and } B \text{ (blue), each consisting of values in the range } [0, 255). \\
& \\
&\text{Consider a file } F \text{, where each bit is denoted by } b. \text{ Let } \{p_{i,j}^{(l)}\} \text{ represent a series of pixels in the image, where } i \text{ is the x-coordinate, } j \text{ is the y-coordinate, and } l \text{ is the length of the series.} \\
& \\
&\text{Define the operation } \mathcal{B} \text{ that extracts 2 bits from each } p_{ij}^{(l)} \text{ and replaces these 2 bits with the corresponding bits from the last channel } (c_b) \text{ of the pixel. This process is iteratively applied until all bits from the file } F \text{ are embedded into the image.} \\
& \\
&\text{This can be expressed as follows:} \\
& \\
&\begin{aligned}
&\mathcal{B}: \{p_{ij}^{(l)}\} \rightarrow \{p_{ij}^{(l)}\}, \text{ where} \\
&p_{ij}^{(l)} = (p_{ij}^{(l)R}, p_{ij}^{(l)G}, p_{ij}^{(l)B}) \\
&\mathcal{B}(x) = x \% 4 \\
&\text{For } m = 1, 2, \ldots, n, \text{ the operation is given by} \\
&\quad b_m \rightarrow \sum_{k=1}^{n} \mathcal{B}(p_{ij}^{(l)B_k})
\end{aligned}
\end{align*}
$$
